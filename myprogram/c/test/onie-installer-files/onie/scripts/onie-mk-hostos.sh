...
if  [ ! -d $installer_dir ] || \
    [ ! -r $installer_dir/sharch_body_hostos.sh ] ; then
    echo "Error: Invalid installer script directory: $installer_dir"
    exit 1
fi

if  [ ! -d $installer_dir/$arch ] || \
    [ ! -r $installer_dir/$arch/install_hostos.sh ] ; then
...
cp $installer_dir/$arch/install_hostos.sh $tmp_installdir || clean_up 1
...
sed -i -e "s/%%DEMO_TYPE%%/$demo_type/g" \
    ...
    $tmp_installdir/install_hostos.sh || clean_up 1
echo -n "."
...
echo -n "."
cp $installer_dir/sharch_body_hostos.sh $output_file || {
